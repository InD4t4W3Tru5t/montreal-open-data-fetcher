...
st.set_page_config(page_title="🗺️ Montréal Open Data Explorer", page_icon="🗺️", layout="wide")

TRANSLATIONS = {
    "en": {
        "page_title": "🗺️ Montréal Open Data Explorer",
        "page_subtitle": "Browse and download datasets from the **[Ville de Montréal Open Data portal](https://donnees.montreal.ca)**.",
        "sidebar_header": "⚙️ Configuration",
        "nav_browser": "📚 Dataset Browser",
        "nav_fetcher": "🔍 Fetch by Resource ID",
        "sidebar_caption": "Data source: [Données ouvertes – Ville de Montréal](https://donnees.montreal.ca)  
Built with Streamlit · [Source code on GitHub](https://github.com)",
        ...
        "how_to_body": "1. Use the **Dataset Browser** tab on the left
...
4. Copy the UUID from the URL: `.../resource/<resource-id>`",
    ...
    "fr": {
        ...
        "page_subtitle": "Parcourez et téléchargez les jeux de données du **[portail de données ouvertes de la Ville de Montréal](https://donnees.montreal.ca)**.",
        "sidebar_header": "⚙️ Configuration",
        "nav_browser": "📚 Navigateur de données",
        "nav_fetcher": "🔍 Récupérer par identifiant",
        "sidebar_caption": "Source : [Données ouvertes – Ville de Montréal](https://donnees.montreal.ca)  
Construit avec Streamlit · [Code source sur GitHub](https://github.com)",
        ...
        "fetcher_subtitle": "Entrez n'importe quel identifiant de ressource du portail de données ouvertes de Montréal pour prévisualiser et télécharger le jeu de données.",
        ...
        "how_to_body": "1. Utilisez l'onglet **Navigateur de données** à gauche
...
def safe_slider(total: int):
    ...
    return st.slider(
        ...
        value=slider_val, step=slider_step,
...
def load_catalog():
    ...
    while True:
        try:
            resp = requests.get(PACKAGE_SEARCH,
            ...
            start += len(results)
            if start >= data["result"]["count"]: break
            time.sleep(0.2)
...
def get_resource_total(resource_id):
    try:
        resp = requests.get(BASE_URL,
            params={"resource_id": resource_id, "limit": 1, "offset": 0}, timeout=20)
        resp.raise_for_status()
        data = resp.json()
        if data.get("success"): return data["result"]["total"]
...
def download_raw_file(url: str, filename: str, fmt: str):
    try:
        ...
        st.download_button(
            ...
            data=file_bytes, file_name=filename, mime=mime,
...
def fetch_all_records(resource_id, max_rows=None):
    ...
    while True:
        ...
        if data is None or not data.get("success"):
            ...
            return None
        result = data["result"]
        if total is None: total = result["total"]
...
def render_data_panel(df, resource_id, dataset_name):
    ...
    if search_term:
        ...
        display_df = df
    # ✅ st.dataframe supports width="stretch"
    ...
    with st.expander(t("col_info_expander")):
        col_info = pd.DataFrame({
            ...
            t("col_sample"):  [str(df[c].dropna().iloc[0]) if df[c].notna().any() else "N/A" for c in df.columns],
        })
        # ✅ st.dataframe supports width="stretch"
    ...
    # ✅ st.download_button uses use_container_width (width= NOT supported)
    st.download_button(
        label=t("download_btn"), data=csv_bytes, file_name=filename,
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    # ✅ st.button uses use_container_width (width= NOT supported)
    ...
    page = st.radio(
        label="Navigation", options=["browser", "fetcher"],
    ...
    if page == "fetcher":
        ...
        resource_id_input = st.text_input(
            label=t("resource_id_label"), value="cc41b532-f12d-40fb-9f55-eb58c9a2b12b",
            placeholder=t("resource_id_placeholder"), help=t("resource_id_help"),
        )
...
if page == "browser":
    ...
    search_query = st.text_input(t("search_datasets"), placeholder=t("search_placeholder"), key="catalog_search")
    ...
    selected_title = st.selectbox(label=t("select_dataset"), options=dataset_titles, key="pkg_selectbox")
    ...
    selected_pkg = next((p for p in catalog if p.get("title", p.get("name")) == selected_title), None)
    if selected_pkg:
        ...
        if resources:
            ...
            selected_res_label = st.selectbox(label=t("fetch_this"), options=res_labels, key="res_selectbox")
            ...
            if res_fmt and not is_tabular(res_fmt):
                ...
                st.subheader(t("download_header"))
                filename = f"{res_name.replace(' ', '_')}_{rid[:8]}.{res_fmt.lower()}"
                ...
                if total_count and total_count > 0:
                    ...
                    if total_count >= SLIDER_THRESHOLD:
                        limit_rows_browser = st.checkbox(t("limit_rows_browser"), value=True, key="limit_rows_browser_cb")
    ...
    if fetch_btn:
        if not resource_id_input.strip():
            ...
            if res_fmt_upper and not is_tabular(res_fmt_upper):
                ...
                st.subheader(t("download_header"))
                filename = f"{res_name.replace(' ', '_')}_{rid[:8]}.{res_fmt_upper.lower()}"
